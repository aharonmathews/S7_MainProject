...
class TFIDFContentCurator:
    ...
    def recommend_content(self, user_preferences, top_k=10):
        ...
        similarities = cosine_similarity(user_vector, self.content_embeddings)
        recommended_indices = similarities.argsort()[0][-top_k:][::-1]
        return [(self.content_ids[i], similarities[0][i]) for i in recommended_indices]